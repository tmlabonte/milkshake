...
class SwinTransformer(Model):
    ...
    def __init__(self, args):
        ...
        if args.swin_transformer_pretrained:
            self.model = swin_transformers[args.swin_transformer_version](weights="IMAGENET1K_V1")
        else:
            self.model = swin_transformers[args.swin_transformer_version](weights=None)

        self.model.head = nn.Sequential(
            nn.Dropout(p=args.dropout_prob),
            nn.Linear(self.model.head.in_features, args.num_classes),
        )
